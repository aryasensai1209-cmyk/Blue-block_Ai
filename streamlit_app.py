...
# ---- session-added rules (batch 1 + batch 2), merged inline per request ----
VULNERABILITY_RULES += [
    {
        "id": 'SEC-081', "title": 'SQL-shaped string built with %-formatting, regardless of where execute() is called',
        "pattern": '[\\"\'](?:SELECT|INSERT|UPDATE|DELETE)\\b.*?%s.*?[\\"\']\\s*%\\s*\\w',
        "severity": 'Critical', "cwe": 'CWE-89', "cvss": 9.1,
        "description": "A SQL-keyword string is built with %-formatting and assigned or passed on, independent of whether execute() appears on the same line — catches the common 'build query, then execute(query)' two-step pattern the direct-call rules miss.",
        "remediation": 'Use parameterized queries (cursor.execute(sql, params)) instead of building SQL text with % or f-strings, no matter how many steps separate the build from the execute call.',
    },
    {
        "id": 'SEC-082', "title": 'SQL-shaped f-string interpolation, regardless of where execute() is called',
        "pattern": 'f[\\"\'](?:SELECT|INSERT|UPDATE|DELETE)\\b.*?\\{[^}]+\\}',
        "severity": 'Critical', "cwe": 'CWE-89', "cvss": 9.1,
        "description": 'An f-string containing a SQL keyword interpolates a variable directly into the query text — same two-step blind spot as SEC-081 but for f-strings.',
        "remediation": 'Never interpolate variables into SQL text with f-strings; use parameter placeholders (?, %s, :name) and pass values separately.',
    },
    {
        "id": 'SEC-083', "title": 'yaml.load() without a safe Loader',
        "pattern": 'yaml\\.load\\((?![^)]*Loader)[^)]*\\)',
        "severity": 'Critical', "cwe": 'CWE-502', "cvss": 8.6,
        "description": 'yaml.load() without an explicit Loader= argument uses the full (unsafe) loader by default in older PyYAML, allowing arbitrary Python object construction from untrusted YAML.',
        "remediation": 'Use yaml.safe_load(), or yaml.load(data, Loader=yaml.SafeLoader) explicitly.',
    },
    {
        "id": 'SEC-084', "title": 'Flask app run with debug=True',
        "pattern": '\\.run\\s*\\([^)]*debug\\s*=\\s*True',
        "severity": 'High', "cwe": 'CWE-16', "cvss": 7.5,
        "description": "Flask's debug mode exposes the Werkzeug interactive debugger, which allows arbitrary code execution if it's ever reachable from a network the attacker can reach.",
        "remediation": 'Never enable debug=True outside local development; use an environment-variable-gated flag that defaults to False and is unset in production.',
    },
    {
        "id": 'SEC-085', "title": 'Django SECRET_KEY left at an insecure default value',
        "pattern": 'SECRET_KEY\\s*=\\s*[\\"\'](?:django-insecure|changeme|secret|123|CHANGE_ME)',
        "severity": 'Critical', "cwe": 'CWE-798', "cvss": 9.1,
        "description": 'The Django SECRET_KEY is set to a well-known placeholder value (e.g. the django-admin startproject default), which undermines session signing, password reset tokens, and CSRF protection.',
        "remediation": 'Generate a unique, random SECRET_KEY per environment and load it from an environment variable or secrets manager — never commit it to source control.',
    },
    {
        "id": 'SEC-086', "title": 'assert used for an authorization/authentication check',
        "pattern": 'assert\\s+\\w+\\.(?:is_admin|authenticated|is_authenticated|has_permission|is_authorized)\\b',
        "severity": 'High', "cwe": 'CWE-703', "cvss": 7.3,
        "description": 'assert statements are stripped when Python runs with the -O optimization flag, silently removing the security check in optimized deployments.',
        "remediation": 'Use an explicit `if not check: raise PermissionError(...)` instead of assert for any authorization or authentication decision.',
    },
    {
        "id": 'SEC-087', "title": 'Django ALLOWED_HOSTS wildcard',
        "pattern": 'ALLOWED_HOSTS\\s*=\\s*\\[\\s*[\\"\']\\*[\\"\']\\s*\\]',
        "severity": 'Medium', "cwe": 'CWE-16', "cvss": 5.3,
        "description": "A wildcard ALLOWED_HOSTS disables Django's Host header validation, which can enable HTTP Host header poisoning (password-reset link poisoning, cache poisoning).",
        "remediation": 'List the specific hostnames the app is served on; use environment-specific settings rather than a wildcard.',
    },
    {
        "id": 'SEC-088', "title": 'Hardcoded AWS Access Key ID',
        "pattern": '\\bAKIA[0-9A-Z]{16}\\b',
        "severity": 'Critical', "cwe": 'CWE-798', "cvss": 9.8,
        "description": 'An AWS Access Key ID (AKIA... format) is hardcoded in source.',
        "remediation": 'Revoke and rotate the key immediately, remove it from git history, and load credentials via IAM roles, environment variables, or a secrets manager instead.',
    },
    {
        "id": 'SEC-089', "title": 'Hardcoded AWS Secret Access Key',
        "pattern": 'aws_secret_access_key\\s*[=:]\\s*[\\"\'][A-Za-z0-9/+=]{40}[\\"\']',
        "severity": 'Critical', "cwe": 'CWE-798', "cvss": 9.8,
        "description": 'A 40-character AWS secret access key is hardcoded next to its recognizable variable name.',
        "remediation": 'Rotate the key immediately and load it from an environment variable, AWS Secrets Manager, or an IAM role — never from source.',
    },
    {
        "id": 'SEC-090', "title": 'Hardcoded GitHub personal access token (classic)',
        "pattern": '\\bghp_[A-Za-z0-9]{36}\\b',
        "severity": 'Critical', "cwe": 'CWE-798', "cvss": 9.1,
        "description": 'A classic GitHub personal access token (ghp_ prefix) is hardcoded in source.',
        "remediation": 'Revoke the token in GitHub settings immediately and switch to a short-lived token or GitHub App installation token loaded from a secrets store.',
    },
    {
        "id": 'SEC-091', "title": 'Hardcoded GitHub fine-grained access token',
        "pattern": '\\bgithub_pat_[A-Za-z0-9_]{22,}\\b',
        "severity": 'Critical', "cwe": 'CWE-798', "cvss": 9.1,
        "description": 'A GitHub fine-grained personal access token is hardcoded in source.',
        "remediation": 'Revoke the token immediately and load it from a secrets manager or CI/CD secret store.',
    },
    {
        "id": 'SEC-092', "title": 'Hardcoded GitLab personal access token',
        "pattern": '\\bglpat-[A-Za-z0-9\\-_]{20}\\b',
        "severity": 'Critical', "cwe": 'CWE-798', "cvss": 9.1,
        "description": 'A GitLab personal access token (glpat- prefix) is hardcoded in source.',
        "remediation": 'Revoke the token in GitLab settings and load it from CI/CD variables or a secrets manager.',
    },
    {
        "id": 'SEC-093', "title": 'Hardcoded Slack token',
        "pattern": '\\bxox[baprs]-[A-Za-z0-9\\-]{10,}\\b',
        "severity": 'High', "cwe": 'CWE-798', "cvss": 8.1,
        "description": 'A Slack API token (xoxb/xoxp/xoxa/xoxr/xoxs prefix) is hardcoded in source.',
        "remediation": 'Revoke the token in the Slack app settings and load it from environment variables or a secrets manager.',
    },
    {
        "id": 'SEC-094', "title": 'Hardcoded Slack incoming webhook URL',
        "pattern": 'hooks\\.slack\\.com/services/[A-Za-z0-9/]{20,}',
        "severity": 'Medium', "cwe": 'CWE-798', "cvss": 6.5,
        "description": 'A Slack incoming webhook URL is hardcoded — anyone with this URL can post messages to the associated channel.',
        "remediation": 'Regenerate the webhook and load its URL from configuration rather than source.',
    },
    {
        "id": 'SEC-095', "title": 'Hardcoded Stripe live secret key',
        "pattern": '\\bsk_live_[A-Za-z0-9]{24,}\\b',
        "severity": 'Critical', "cwe": 'CWE-798', "cvss": 9.8,
        "description": 'A Stripe LIVE secret key is hardcoded — this key can move real money and access customer payment data.',
        "remediation": 'Roll the key immediately in the Stripe dashboard and load it from an environment variable or secrets manager.',
    },
    {
        "id": 'SEC-096', "title": 'Hardcoded Stripe restricted key',
        "pattern": '\\brk_live_[A-Za-z0-9]{24,}\\b',
        "severity": 'Critical', "cwe": 'CWE-798', "cvss": 9.1,
        "description": 'A Stripe restricted (scoped) live API key is hardcoded in source.',
        "remediation": 'Roll the key in the Stripe dashboard and load it from a secrets manager.',
    },
    {
        "id": 'SEC-097', "title": 'Hardcoded SendGrid API key',
        "pattern": '\\bSG\\.[A-Za-z0-9_\\-]{22}\\.[A-Za-z0-9_\\-]{43}\\b',
        "severity": 'High', "cwe": 'CWE-798', "cvss": 8.1,
        "description": 'A SendGrid API key is hardcoded in source.',
        "remediation": 'Revoke the key in SendGrid settings and load it from environment configuration.',
    },
    {
        "id": 'SEC-098', "title": 'Hardcoded Google API key',
        "pattern": '\\bAIza[0-9A-Za-z\\-_]{35}\\b',
        "severity": 'High', "cwe": 'CWE-798', "cvss": 7.5,
        "description": "A Google API key (AIza... format) is hardcoded in source — depending on which APIs it's scoped to, this can allow abuse billed to the project owner.",
        "remediation": "Restrict the key's scope/referrers in Google Cloud Console, rotate it, and load it from configuration rather than source.",
    },
    {
        "id": 'SEC-099', "title": 'Hardcoded Google OAuth client secret',
        "pattern": '\\bGOCSPX-[A-Za-z0-9_\\-]{28}\\b',
        "severity": 'Critical', "cwe": 'CWE-798', "cvss": 8.6,
        "description": 'A Google OAuth 2.0 client secret is hardcoded in source.',
        "remediation": 'Reset the client secret in Google Cloud Console and load it from a secrets manager.',
    },
    {
        "id": 'SEC-100', "title": 'Hardcoded private key material',
        "pattern": '-----BEGIN (?:RSA |EC |OPENSSH |DSA )?PRIVATE KEY-----',
        "severity": 'Critical', "cwe": 'CWE-798', "cvss": 9.8,
        "description": 'A PEM-format private key is embedded directly in source.',
    ...
    {
        "id": 'SEC-101', "title": 'Hardcoded npm access token',
        "pattern": '\\bnpm_[A-Za-z0-9]{36}\\b',
        "severity": 'High', "cwe": 'CWE-798', "cvss": 8.1,
        "description": 'An npm access token is hardcoded — this can allow publishing packages under the associated account.',
        "remediation": 'Revoke the token on npmjs.com and use CI/CD secret storage (e.g. NODE_AUTH_TOKEN from a secret) instead.',
    },
    {
        "id": 'SEC-102', "title": 'Hardcoded PyPI upload token',
        "pattern": '\\bpypi-AgEIcHlwaS5vcmc[A-Za-z0-9\\-_]{20,}\\b',
        "severity": 'High', "cwe": 'CWE-798', "cvss": 8.1,
        "description": 'A PyPI upload token is hardcoded in source.',
        "remediation": 'Revoke the token on pypi.org and store it as a CI/CD secret instead.',
    },
    {
        "id": 'SEC-103', "title": 'Likely hardcoded Twilio API key SID',
        "pattern": '\\bSK[a-f0-9]{32}\\b',
        "severity": 'Medium', "cwe": 'CWE-798', "cvss": 6.5,
        "description": "A string matching Twilio's API key SID format (SK followed by 32 hex characters) is present — this is the identifier half of a Twilio API key pair.",
        "remediation": "If this is a live key, confirm the matching secret isn't also hardcoded nearby, rotate both in the Twilio console, and load them from configuration.",
    },
    {
        "id": 'SEC-104', "title": 'Likely hardcoded Mailgun API key',
        "pattern": '\\bkey-[0-9a-f]{32}\\b',
        "severity": 'Medium', "cwe": 'CWE-798', "cvss": 6.5,
        "description": "A string matching Mailgun's legacy API key format (key- followed by 32 hex characters) is present in source.",
        "remediation": 'Rotate the key in the Mailgun dashboard if live, and load it from environment configuration.',
    },
    {
        "id": 'SEC-105', "title": 'Security group / firewall rule open to the entire internet',
        "pattern": 'cidr_blocks\\s*=\\s*\\[\\s*[\\"\']0\\.0\\.0\\.0/0[\\"\']',
        "severity": 'High', "cwe": 'CWE-284', "cvss": 7.5,
        "description": 'A security group or firewall rule allows traffic from 0.0.0.0/0 (any IPv4 address).',
        "remediation": 'Scope cidr_blocks to the specific IP ranges that need access; use a bastion/VPN for admin access instead of opening it to the internet.',
    },
    {
        "id": 'SEC-106', "title": 'Database instance marked publicly accessible',
        "pattern": 'publicly_accessible\\s*=\\s*true',
        "severity": 'Critical', "cwe": 'CWE-284', "cvss": 9.1,
        "description": 'An RDS/database resource has publicly_accessible = true, exposing it directly to the internet.',
        "remediation": 'Set publicly_accessible = false and access the database through a VPC/private subnet, bastion host, or VPN.',
    },
    {
        "id": 'SEC-107', "title": 'Storage encryption explicitly disabled',
        "pattern": 'storage_encrypted\\s*=\\s*false',
        "severity": 'High', "cwe": 'CWE-311', "cvss": 7.5,
        "description": 'storage_encrypted is explicitly set to false on a database/storage resource.',
        "remediation": "Remove the override (or set storage_encrypted = true) — there's essentially never a good reason to store data unencrypted at rest.",
    },
    {
        "id": 'SEC-108', "title": 'IAM policy grants wildcard Action and Resource',
        "pattern": '\\"Action\\"\\s*:\\s*\\"\\*\\"[\\s\\S]{0,120}\\"Resource\\"\\s*:\\s*\\"\\*\\"',
        "severity": 'Critical', "cwe": 'CWE-284', "cvss": 9.1,
        "description": 'An IAM policy document grants "Action": "*" over "Resource": "*" — full administrative access to everything in the account.',
        "remediation": 'Scope the policy to the specific actions and resource ARNs the role/user actually needs (least privilege).',
    },
    {
        "id": 'SEC-109', "title": 'force_destroy enabled on a storage resource',
        "pattern": 'force_destroy\\s*=\\s*true',
        "severity": 'Medium', "cwe": 'CWE-668', "cvss": 5.3,
        "description": 'force_destroy = true allows `terraform destroy` to delete a bucket/resource even while it still contains data, with no confirmation.',
        "remediation": 'Only set force_destroy = true for genuinely disposable/ephemeral resources (e.g. CI scratch buckets) — never for anything holding real data.',
    },
    {
        "id": 'SEC-110', "title": 'Security group egress unrestricted on all ports',
        "pattern": 'from_port\\s*=\\s*0[\\s\\S]{0,200}to_port\\s*=\\s*65535',
        "severity": 'Medium', "cwe": 'CWE-284', "cvss": 6.5,
        "description": 'A security group rule spans the full port range (0-65535), which for egress in particular makes data-exfiltration channels harder to detect/block.',
        "remediation": 'Scope egress rules to the specific ports the workload actually needs to reach outbound.',
    },
    {
        "id": 'SEC-111', "title": 'Disk/volume encryption explicitly disabled',
        "pattern": '\\bencrypted\\s*=\\s*false',
        "severity": 'High', "cwe": 'CWE-311', "cvss": 7.5,
        "description": 'An EBS volume or similar block-storage resource has encryption explicitly disabled.',
        "remediation": 'Remove the override so the volume is encrypted at rest; most cloud providers default to free, transparent volume encryption.',
    },
    {
        "id": 'SEC-112', "title": 'Archive extraction without member-path validation (Zip Slip)',
        "pattern": '\\.extractall\\(\\s*\\)',
        "severity": 'High', "cwe": 'CWE-22', "cvss": 7.5,
        "description": "A bare .extractall() call (zipfile or tarfile) extracts every archive member using its stored path as-is — a malicious archive with '../' in an entry name can write outside the target directory.",
        "remediation": "Validate each member's resolved path stays inside the target directory before extracting (e.g. check os.path.realpath(...) against the target root), or extract members individually with sanitized names.",
    },
    {
        "id": 'SEC-113', "title": 'Unsanitized request data written directly to a log statement',
        "pattern": 'log(?:ger)?\\.\\w+\\(\\s*(?:f[\\"\'][^\\"\']*\\{request\\.|[^)]*\\+\\s*request\\.)',
        "severity": 'Medium', "cwe": 'CWE-117', "cvss": 5.3,
        "description": 'Request data is interpolated directly into a log message; if it contains newlines or log-format control characters, an attacker can forge additional fake log entries.',
        "remediation": 'Sanitize/strip control characters (in particular \\r\\n) from user-controlled values before logging them, or use structured logging with the value as a separate field rather than inline text.',
    },
    {
        "id": 'SEC-114', "title": 'Secret-like value compared with a non-constant-time operator',
        "pattern": '\\b(?:if|while)\\s+\\w*(?:token|secret|password|signature|hmac)\\w*\\s*==\\s*\\w',
        "severity": 'Medium', "cwe": 'CWE-208', "cvss": 5.9,
        "description": 'A variable whose name suggests a secret/token/signature is compared with == , which short-circuits on the first differing byte — an attacker who can measure response timing may be able to guess the value byte-by-byte.',
        "remediation": "Use a constant-time comparison (e.g. Python's hmac.compare_digest, Java's MessageDigest.isEqual) for any secret, token, or signature comparison.",
    },
    {
        "id": 'SEC-115', "title": 'HTTP header value built from unsanitized request data',
        "pattern": '(?:set_header|setHeader|add_header)\\s*\\([^)]*request\\.',
        "severity": 'Medium', "cwe": 'CWE-93', "cvss": 6.1,
        "description": 'A response header value is built directly from request data; if it contains \\r\\n, an attacker may be able to inject additional headers or split the response (HTTP response splitting).',
        "remediation": "Strip/reject CR and LF characters from any value before using it in a header, or use your framework's header-setting API (most already do this validation for you — confirm this one does too).",
    },
    {
        "id": 'SEC-116', "title": 'Hardcoded/static encryption IV',
        "pattern": '\\biv\\s*=\\s*b?[\\"\'][0-9a-fA-F]{16,32}[\\"\']',
        "severity": 'Medium', "cwe": 'CWE-329', "cvss": 5.9,
        "description": 'The initialization vector (IV) for a block cipher appears to be a fixed, hardcoded value. Reusing the same IV across encryptions (especially with CBC or CTR mode) can leak information about the plaintext or fully break confidentiality.',
        "remediation": "Generate a fresh, random IV for every encryption operation (e.g. os.urandom(16)) and store/transmit it alongside the ciphertext — it doesn't need to be secret, just unique.",
    },
    {
        "id": 'SEC-117', "title": 'Reference to the cloud instance-metadata endpoint',
        "pattern": '169\\.254\\.169\\.254|metadata\\.google\\.internal',
        "severity": 'Critical', "cwe": 'CWE-918', "cvss": 8.6,
        "description": "Code references the cloud provider's instance-metadata IP/hostname directly. If this address is reachable from a URL an attacker can influence (classic SSRF), it can be used to steal instance IAM credentials.",
        "remediation": "If this is an SSRF-protection allowlist/denylist, keep it — that's the right use. If it's a URL being requested with any attacker-influenced portion, block requests to link-local/metadata addresses at the network layer, not just in application code.",
    },
    {
        "id": 'SEC-118', "title": 'JWT decoded without verifying the signature',
        "pattern": 'jwt\\.decode\\([^)]*verify(?:_signature)?\\s*=\\s*False',
        "severity": 'Critical', "cwe": 'CWE-347', "cvss": 9.1,
        "description": 'A JWT is decoded with signature verification explicitly disabled, so any caller can forge a token with arbitrary claims.',
        "remediation": 'Always verify the signature (remove the verify=False override) and explicitly pin the expected algorithm(s) to prevent algorithm-confusion attacks.',
    },
    {
        "id": 'SEC-119', "title": 'os.popen() used to run a shell command',
        "pattern": 'os\\.popen\\s*\\(',
        "severity": 'High', "cwe": 'CWE-78', "cvss": 8.1,
        "description": 'os.popen() runs its argument through the system shell, the same risk class as os.system()/subprocess with shell=True.',
        "remediation": 'Use subprocess.run([...], shell=False) with an argument list instead of a shell string.',
    },
    {
        "id": 'SEC-120', "title": 'Regular expression pattern built from request data',
        "pattern": 're\\.(?:compile|match|search|fullmatch)\\(\\s*request\\.',
        "severity": 'Medium', "cwe": 'CWE-1333', "cvss": 5.3,
        "description": 'A regex PATTERN (not just the subject text) is taken directly from request data — an attacker who controls the pattern can supply a catastrophically-backtracking one and hang the process (ReDoS), independent of any bug in your own regexes.',
        "remediation": 'Never let external input become a regex pattern. If pattern-like matching is genuinely needed from user input, use a non-backtracking approach (e.g. fnmatch for globs, or a regex engine with guaranteed linear-time matching).',
    },
    {
        "id": 'SEC-121', "title": 'marshal.loads() used to deserialize data',
        "pattern": 'marshal\\.loads?\\(',
        "severity": 'High', "cwe": 'CWE-502', "cvss": 8.1,
        "description": "Python's marshal module is intended for internal bytecode serialization, not general-purpose data exchange — it has no protection against malicious input and can crash the interpreter or worse.",
        "remediation": 'Use json (untrusted data) or a signed/authenticated format if the data must be trusted; avoid marshal and pickle for anything crossing a trust boundary.',
    },
]


# ---- batch 3 (session 3): misc cross-language rules ----
VULNERABILITY_RULES += [
    {
        "id": 'SEC-122', "title": 'Email header built from unsanitized request data',
        "pattern": '(?:msg\\[.Subject.\\]|Subject\\s*=)\\s*.*request\\.',
        "severity": 'Medium', "cwe": 'CWE-93', "cvss": 6.1,
        "description": 'An email Subject (or similar header) is built directly from request data; a newline in the value can inject additional headers or recipients (email header injection).',
        "remediation": "Strip CR/LF characters from any user-controlled value before using it in an email header, or use a mail library's structured header-setting API rather than string formatting.",
    },
    {
        "id": 'SEC-123', "title": 'Secret-key environment lookup has a hardcoded fallback value',
        "pattern": 'secret_key\\s*=\\s*os\\.environ\\.get\\([^,]+,\\s*[\\"\'][^\\"\']{4,}[\\"\']\\)',
        "severity": 'High', "cwe": 'CWE-798', "cvss": 7.5,
        "description": 'os.environ.get("SECRET_KEY", "<fallback>") silently falls back to a hardcoded value whenever the environment variable isn\'t set, which is exactly the failure mode most likely to happen by accident in a misconfigured deployment.',
        "remediation": "Fail startup loudly (raise / sys.exit) if the secret isn't provided via environment/secrets manager, rather than silently substituting a hardcoded fallback.",
    },
    {
        "id": 'SEC-124', "title": 'CORS wildcard origin combined with credentials allowed',
        "pattern": 'Access-Control-Allow-Credentials[\\"\']?\\s*[:,]\\s*[\\"\']?true[\\"\']?[\\s\\S]{0,150}Access-Control-Allow-Origin[\\"\']?\\s*[:,]\\s*[\\"\']?\\*',
        "severity": 'High', "cwe": 'CWE-942', "cvss": 7.5,
        "description": "Access-Control-Allow-Credentials: true combined with a wildcard Allow-Origin lets any website make authenticated, credentialed requests on a logged-in user's behalf.",
        "remediation": 'Never combine a wildcard origin with credentials; echo back a specific allow-listed origin instead.',
    },
    {
        "id": 'SEC-125', "title": 'Path built with pathlib directly from request data',
        "pattern": 'Path\\([^)]*\\)\\s*/\\s*request\\.',
        "severity": 'High', "cwe": 'CWE-22', "cvss": 7.5,
        "description": "A pathlib Path is joined directly with request-controlled data; '../' segments in the value can escape the intended base directory.",
        "remediation": "Resolve the final path (Path(...).resolve()) and verify it's still inside the intended base directory before using it, or strip path separators from the input.",
    },
    {
        "id": 'SEC-126', "title": 'Session or CSRF cookie explicitly marked non-secure',
        "pattern": '(?:SESSION_COOKIE_SECURE|CSRF_COOKIE_SECURE)\\s*=\\s*False',
        "severity": 'Medium', "cwe": 'CWE-614', "cvss": 5.9,
        "description": "Django's SESSION_COOKIE_SECURE / CSRF_COOKIE_SECURE is explicitly disabled, allowing the cookie to be sent over plain HTTP.",
        "remediation": 'Set these to True in any environment served over HTTPS (which should be all of them in production).',
    },
    {
        "id": 'SEC-127', "title": 'PyYAML unsafe_load() used explicitly',
        "pattern": 'yaml\\.unsafe_load\\s*\\(',
        "severity": 'Critical', "cwe": 'CWE-502', "cvss": 8.6,
        "description": "yaml.unsafe_load() is PyYAML's explicitly-named dangerous loader — it allows arbitrary Python object construction from the YAML content, equivalent to the old unrestricted yaml.load().",
        "remediation": 'Use yaml.safe_load() unless you have a specific, well-understood need to construct arbitrary Python objects from trusted YAML only.',
    },
    {
        "id": 'SEC-129', "title": 'Django mark_safe() applied to request-derived data',
        "pattern": 'mark_safe\\([^)]*request\\.',
        "severity": 'Critical', "cwe": 'CWE-79', "cvss": 8.8,
        "description": "mark_safe() tells Django's template engine to skip auto-escaping for this value entirely — applying it to request data reintroduces XSS that auto-escaping exists to prevent.",
        "remediation": 'Never wrap request-derived data in mark_safe(); if HTML needs to be preserved, sanitize it first with a library like bleach.',
    },
]
...
# ---- session-added pattern rules (batch 1 + batch 2), merged inline per request ----
PATTERN_RULES += [
    PatternRule(id='PH-007', title='CORS wildcard origin', pattern='header\\s*\\(\\s*[\\"\']Access-Control-Allow-Origin:\\s*\\*[\\"\']', language='php',
                severity=Severity('Medium'), confidence=Confidence('High'),
                standards=_std('CWE-942', 'A05:2021-Security Misconfiguration', 'ASVS 14.5.3', 'CAPEC-111', 'PW.5.1'),
                remediation="Echo back a specific allow-listed origin instead of '*', especially on any endpoint that also sets Access-Control-Allow-Credentials."),
    PatternRule(id='PH-008', title='Session cookie explicitly set to non-HttpOnly', pattern='ini_set\\s*\\(\\s*[\\"\']session\\.cookie_httponly[\\"\']\\s*,\\s*[\\"\']?0', language='php',
                severity=Severity('Medium'), confidence=Confidence('High'),
                standards=_std('CWE-614', 'A05:2021-Security Misconfiguration', 'ASVS 3.4.1', 'CAPEC-31', 'PW.5.1'),
                remediation='Set session.cookie_httponly to 1 (or remove the override so the secure default applies) to block JavaScript access to the session cookie.'),
    PatternRule(id='PH-009', title='Open redirect via unsanitized Location header', pattern='header\\s*\\(\\s*[\\"\']Location:\\s*[\\"\']\\s*\\.\\s*\\$_(?:GET|POST|REQUEST)', language='php',
                severity=Severity('Medium'), confidence=Confidence('Medium'),
                standards=_std('CWE-601', 'A01:2021-Broken Access Control', 'ASVS 5.1.5', 'CAPEC-178', 'PW.5.1'),
                remediation='Validate the redirect target against an allow-list of paths/hosts before using it in a Location header.'),
    PatternRule(id='PH-010', title='cURL SSL certificate verification disabled', pattern='curl_setopt\\s*\\(\\s*\\$\\w+\\s*,\\s*CURLOPT_SSL_VERIFYPEER\\s*,\\s*(?:false|0)\\s*\\)', language='php',
                severity=Severity('High'), confidence=Confidence('High'),
                standards=_std('CWE-295', 'A02:2021-Cryptographic Failures', 'ASVS 9.1.1', 'CAPEC-94', 'PW.5.1'),
                remediation='Remove the override so CURLOPT_SSL_VERIFYPEER stays enabled (default), and fix the underlying certificate issue instead of disabling verification.'),
    PatternRule(id='PH-011', title='File upload trusts the client-supplied filename', pattern='move_uploaded_file\\s*\\(\\s*\\$_FILES\\[[^\\]]+\\]\\[[\\"\']tmp_name[\\"\']\\]\\s*,\\s*\\$_FILES\\[[^\\]]+\\]\\[[\\"\']name[\\"\']\\]', language='php',
                severity=Severity('High'), confidence=Confidence('Medium'),
                standards=_std('CWE-434', 'A04:2021-Insecure Design', 'ASVS 12.4.1', 'CAPEC-1', 'PW.5.1'),
                remediation="Generate a new filename server-side (e.g. a UUID) and validate the extension/MIME type against an allow-list rather than trusting $_FILES[...]['name']."),
    PatternRule(id='PH-012', title='MD5 used to hash a password', pattern='md5\\s*\\(\\s*\\$(?:password|pass|pwd)\\b', language='php',
                severity=Severity('High'), confidence=Confidence('Medium'),
                standards=_std('CWE-327', 'A02:2021-Cryptographic Failures', 'ASVS 6.2.1', 'CAPEC-97', 'PW.6.1'),
                remediation='Use password_hash() with PASSWORD_DEFAULT (bcrypt/argon2) instead of a fast general-purpose hash like MD5.'),
    PatternRule(id='JS-007', title='Cookie explicitly set with httpOnly: false', pattern='\\.cookie\\([^)]*httpOnly\\s*:\\s*false', language='javascript',
                severity=Severity('Medium'), confidence=Confidence('High'),
                standards=_std('CWE-614', 'A05:2021-Security Misconfiguration', 'ASVS 3.4.1', 'CAPEC-31', 'PW.5.1'),
                remediation='Remove the override (or set httpOnly: true) unless client-side JS genuinely needs to read this cookie.'),
    PatternRule(id='JS-008', title='CORS wildcard origin set via response header', pattern='setHeader\\s*\\(\\s*[\\"\']Access-Control-Allow-Origin[\\"\']\\s*,\\s*[\\"\']\\*[\\"\']', language='javascript',
                severity=Severity('Medium'), confidence=Confidence('High'),
                standards=_std('CWE-942', 'A05:2021-Security Misconfiguration', 'ASVS 14.5.3', 'CAPEC-111', 'PW.5.1'),
                remediation="Reflect a specific allow-listed origin instead of '*', particularly if Access-Control-Allow-Credentials is also set."),
    PatternRule(id='JS-009', title='Open redirect via unvalidated request parameter', pattern='res\\.redirect\\s*\\(\\s*req\\.(?:query|params|body)', language='javascript',
                severity=Severity('Medium'), confidence=Confidence('Medium'),
                standards=_std('CWE-601', 'A01:2021-Broken Access Control', 'ASVS 5.1.5', 'CAPEC-178', 'PW.5.1'),
                remediation='Validate the redirect target against an allow-list of known-safe paths before passing it to res.redirect().'),
    PatternRule(id='JS-010', title='MongoDB query built with $where from request data', pattern='\\.find\\s*\\(\\s*\\{\\s*\\$where', language='javascript',
                severity=Severity('Critical'), confidence=Confidence('Medium'),
                standards=_std('CWE-943', 'A03:2021-Injection', 'ASVS 5.3.4', 'CAPEC-676', 'PW.5.1'),
                remediation='Avoid $where (it evaluates arbitrary JS server-side); express the query with standard MongoDB query operators instead.'),
    PatternRule(id='JS-011', title='Mongoose/ORM create() called directly with req.body', pattern='\\.create\\s*\\(\\s*req\\.body\\s*\\)', language='javascript',
                severity=Severity('High'), confidence=Confidence('Medium'),
                standards=_std('CWE-915', 'A08:2021-Software and Data Integrity Failures', 'ASVS 5.1.4', 'CAPEC-186', 'PW.5.1'),
                remediation='Explicitly pick allowed fields from req.body (or use a schema/DTO) instead of passing the whole request body into create().'),
    PatternRule(id='JS-012', title='TLS certificate verification disabled', pattern='rejectUnauthorized\\s*:\\s*false', language='javascript',
                severity=Severity('High'), confidence=Confidence('High'),
                standards=_std('CWE-295', 'A02:2021-Cryptographic Failures', 'ASVS 9.1.1', 'CAPEC-94', 'PW.5.1'),
                remediation='Remove the override; fix the underlying certificate (e.g. add the CA to the trust store) instead of disabling verification.'),
    PatternRule(id='JS-013', title='Math.random() used to generate a token/session/password value', pattern='(?:token|session[Ii]d|password)\\s*=\\s*[^;]*Math\\.random\\(\\)', language='javascript',
                severity=Severity('High'), confidence=Confidence('Medium'),
                standards=_std('CWE-330', 'A02:2021-Cryptographic Failures', 'ASVS 6.3.1', 'CAPEC-112', 'PW.5.1'),
                remediation='Use crypto.randomBytes() / crypto.randomUUID() for anything security-sensitive — Math.random() is not cryptographically secure.'),
    PatternRule(id='JS-014', title='Regex literal with nested quantifiers (ReDoS-shaped)', pattern='\\([^()]*[+*]\\)[+*]', language='javascript',
                ...
                remediation='Rewrite the group to avoid nested unbounded quantifiers (e.g. atomic-group equivalent or a possessive-style rewrite), or validate/limit input length before matching.'),
    PatternRule(id='JS-015', title="'as any' type-cast immediately feeding eval()", pattern='eval\\s*\\([^)]*as\\s+any[^)]*\\)', language='javascript',
                severity=Severity('Critical'), confidence=Confidence('Low'),
                standards=_std('CWE-95', 'A03:2021-Injection', 'ASVS 5.2.4', 'CAPEC-242', 'PW.5.1'),
                remediation="TypeScript's type system can't protect a value cast to 'any' — treat it as untrusted and avoid eval() entirely regardless of the cast."),
    PatternRule(id='JV-006', title='Cookie explicitly marked non-HttpOnly', pattern='\\.setHttpOnly\\s*\\(\\s*false\\s*\\)', language='java',
                severity=Severity('Medium'), confidence=Confidence('High'),
                standards=_std('CWE-614', 'A05:2021-Security Misconfiguration', 'ASVS 3.4.1', 'CAPEC-31', 'PW.5.1'),
                remediation='Call setHttpOnly(true) unless client-side JS genuinely needs to read this cookie.'),
    PatternRule(id='JV-007', title='Open redirect via unvalidated request parameter', pattern='sendRedirect\\s*\\(\\s*request\\.getParameter', language='java',
                severity=Severity('Medium'), confidence=Confidence('Medium'),
                standards=_std('CWE-601', 'A01:2021-Broken Access Control', 'ASVS 5.1.5', 'CAPEC-178', 'PW.5.1'),
                remediation='Validate the redirect target against an allow-list before passing it to sendRedirect().'),
    PatternRule(id='JV-008', title='Weak cryptographic primitive (MD5 hash or DES cipher)', pattern='MessageDigest\\.getInstance\\s*\\(\\s*[\\"\']MD5[\\"\']\\)|Cipher\\.getInstance\\s*\\(\\s*[\\"\']DES', language='java',
                severity=Severity('Medium'), confidence=Confidence('High'),
                standards=_std('CWE-327', 'A02:2021-Cryptographic Failures', 'ASVS 6.2.1', 'CAPEC-97', 'PW.6.1'),
                remediation='Use SHA-256 (or better) for hashing and AES/GCM/NoPadding for encryption.'),
    PatternRule(id='JV-009', title='TrustManager with an empty checkServerTrusted (accepts all certs)', pattern='checkServerTrusted\\s*\\([^)]*\\)\\s*\\{\\s*\\}', language='java',
                severity=Severity('Critical'), confidence=Confidence('Medium'),
                standards=_std('CWE-295', 'A02:2021-Cryptographic Failures', 'ASVS 9.1.1', 'CAPEC-94', 'PW.5.1'),
                remediation='Implement real certificate validation (or just use the platform default TrustManager) instead of an empty method body that accepts every certificate.'),
    PatternRule(id='JV-010', title='Spring Security CSRF protection disabled', pattern='\\.csrf\\(\\)\\.disable\\(\\)', language='java',
                severity=Severity('High'), confidence=Confidence('High'),
                standards=_std('CWE-352', 'A01:2021-Broken Access Control', 'ASVS 4.2.2', 'CAPEC-62', 'PW.5.1'),
                remediation='Keep CSRF protection enabled for any endpoint reachable from a browser session; if this is a stateless API using token auth, scope the disable to just those routes rather than globally.'),
    PatternRule(id='JV-011', title='BeanUtils.populate() bound directly to the full request parameter map', pattern='BeanUtils\\.populate\\s*\\(\\s*\\w+\\s*,\\s*request\\.getParameterMap', language='java',
                severity=Severity('High'), confidence=Confidence('Medium'),
                standards=_std('CWE-915', 'A08:2021-Software and Data Integrity Failures', 'ASVS 5.1.4', 'CAPEC-186', 'PW.5.1'),
                remediation='Bind only an explicit allow-list of expected fields instead of the entire parameter map, to prevent attacker-supplied fields from overwriting unintended properties.'),
    PatternRule(id='GO-005', title='TLS certificate verification disabled', pattern='InsecureSkipVerify:\\s*true', language='go',
                severity=Severity('High'), confidence=Confidence('High'),
                standards=_std('CWE-295', 'A02:2021-Cryptographic Failures', 'ASVS 9.1.1', 'CAPEC-94', 'PW.5.1'),
                remediation='Remove InsecureSkipVerify; supply a proper CA pool (or fix the certificate) instead of skipping verification.'),
    PatternRule(id='GO-006', title='Open redirect via unvalidated query parameter', pattern='http\\.Redirect\\s*\\([^)]*r\\.URL\\.Query\\(\\)', language='go',
                severity=Severity('Medium'), confidence=Confidence('Medium'),
                standards=_std('CWE-601', 'A01:2021-Broken Access Control', 'ASVS 5.1.5', 'CAPEC-178', 'PW.5.1'),
                remediation='Validate the redirect target against an allow-list before passing it to http.Redirect().'),
    PatternRule(id='GO-007', title='Cookie explicitly marked non-Secure', pattern='http\\.Cookie\\{[^}]*Secure:\\s*false', language='go',
                severity=Severity('Medium'), confidence=Confidence('High'),
                standards=_std('CWE-614', 'A05:2021-Security Misconfiguration', 'ASVS 3.4.1', 'CAPEC-31', 'PW.5.1'),
                remediation='Set Secure: true (and HttpOnly: true) for any cookie carrying session or auth data.'),
    PatternRule(id='GO-008', title='CORS wildcard origin', pattern='Access-Control-Allow-Origin[\\"\'],\\s*[\\"\']\\*[\\"\']', language='go',
                severity=Severity('Medium'), confidence=Confidence('Medium'),
                standards=_std('CWE-942', 'A05:2021-Security Misconfiguration', 'ASVS 14.5.3', 'CAPEC-111', 'PW.5.1'),
                remediation="Echo back a specific allow-listed origin instead of '*'."),
    PatternRule(id='GO-009', title='SQL query built with fmt.Sprintf', pattern='fmt\\.Sprintf\\s*\\(\\s*[\\"\'](?:SELECT|INSERT|UPDATE|DELETE)\\b', language='go',
                severity=Severity('Critical'), confidence=Confidence('Medium'),
                standards=_std('CWE-89', 'A03:2021-Injection', 'ASVS 5.3.4', 'CAPEC-66', 'PW.5.1'),
                remediation='Use database/sql placeholder parameters ($1/?/:name depending on driver) instead of formatting the query string with fmt.Sprintf.'),
    PatternRule(id='RB-005', title='Open redirect via raw params in redirect_to', pattern='redirect_to\\s+params\\[', language='ruby',
                severity=Severity('Medium'), confidence=Confidence('Medium'),
                standards=_std('CWE-601', 'A01:2021-Broken Access Control', 'ASVS 5.1.5', 'CAPEC-178', 'PW.5.1'),
                remediation='Validate the redirect target against an allow-list before passing it to redirect_to.'),
    PatternRule(id='RB-006', title='Cookie explicitly set with httponly: false', pattern='cookies\\[[^\\]]+\\]\\s*=\\s*\\{[^}]*httponly:\\s*false', language='ruby',
                severity=Severity('Medium'), confidence=Confidence('High'),
                standards=_std('CWE-614', 'A05:2021-Security Misconfiguration', 'ASVS 3.4.1', 'CAPEC-31', 'PW.5.1'),
                remediation='Set httponly: true unless client-side JS genuinely needs to read this cookie.'),
    PatternRule(id='RB-007', title='MD5 used to hash a password', pattern='Digest::MD5\\.(?:hexdigest|digest)\\s*\\([^)]*password', language='ruby',
                severity=Severity('High'), confidence=Confidence('Medium'),
                standards=_std('CWE-327', 'A02:2021-Cryptographic Failures', 'ASVS 6.2.1', 'CAPEC-97', 'PW.6.1'),
                remediation='Use bcrypt (has_secure_password) instead of a fast general-purpose hash like MD5 for passwords.'),
    PatternRule(id='RB-008', title='CSRF token verification explicitly skipped', pattern='skip_before_action\\s+:verify_authenticity_token', language='ruby',
                severity=Severity('High'), confidence=Confidence('High'),
                standards=_std('CWE-352', 'A01:2021-Broken Access Control', 'ASVS 4.2.2', 'CAPEC-62', 'PW.5.1'),
                remediation="Scope the skip as narrowly as possible (e.g. only: [:webhook_action]) — confirm the skipped action doesn't accept browser-session-authenticated state changes."),
    PatternRule(id='RB-009', title='MongoDB/Mongoid query using raw $where', pattern='\\.where\\s*\\(\\s*[\\"\']\\$where', language='ruby',
                severity=Severity('Critical'), confidence=Confidence('Medium'),
                standards=_std('CWE-943', 'A03:2021-Injection', 'ASVS 5.3.4', 'CAPEC-676', 'PW.5.1'),
                remediation='Avoid $where (it evaluates arbitrary JS server-side); express the query with standard Mongoid query operators instead.'),
    PatternRule(id='KT-004', title='TrustManager with an empty checkServerTrusted (accepts all certs)', pattern='checkServerTrusted\\s*\\([^)]*\\)\\s*\\{\\s*\\}', language='kotlin',
                severity=Severity('Critical'), confidence=Confidence('Medium'),
                standards=_std('CWE-295', 'A02:2021-Cryptographic Failures', 'ASVS 9.1.1', 'CAPEC-94', 'PW.5.1'),
                remediation='Implement real certificate validation instead of an empty method body that accepts every certificate.'),
    PatternRule(id='KT-005', title='Room @Query built with string concatenation', pattern='@Query\\s*\\(\\s*[\\"\'][^\\"\']*[\\"\']\\s*\\+\\s*\\w', language='kotlin',
                severity=Severity('Critical'), confidence=Confidence('Medium'),
                standards=_std('CWE-89', 'A03:2021-Injection', 'ASVS 5.3.4', 'CAPEC-66', 'PW.5.1'),
                remediation="Use Room's :paramName bind parameters instead of concatenating values into the @Query string."),
    PatternRule(id='RS-003', title='TLS certificate verification disabled (reqwest)', pattern='danger_accept_invalid_certs\\s*\\(\\s*true\\s*\\)', language='rust',
                severity=Severity('High'), confidence=Confidence('High'),
                standards=_std('CWE-295', 'A02:2021-Cryptographic Failures', 'ASVS 9.1.1', 'CAPEC-94', 'PW.5.1'),
                remediation='Remove this override; fix the underlying certificate/CA trust issue instead of disabling verification.'),
    PatternRule(id='RS-004', title='MD5 used for a password-related value', pattern='md5::compute\\s*\\([^)]*password', language='rust',
                severity=Severity('High'), confidence=Confidence('Medium'),
                standards=_std('CWE-327', 'A02:2021-Cryptographic Failures', 'ASVS 6.2.1', 'CAPEC-97', 'PW.6.1'),
                remediation='Use argon2 or bcrypt for password hashing instead of MD5.'),
    PatternRule(id='C-005', title='malloc() size computed with an unchecked multiplication', pattern='malloc\\s*\\(\\s*\\w+\\s*\\*\\s*\\w+\\s*\\)', language='c',
                severity=Severity('Medium'), confidence=Confidence('Low'),
                standards=_std('CWE-190', 'A06:2021-Vulnerable and Outdated Components', 'ASVS 5.1.1', 'CAPEC-92', 'PW.5.1'),
                remediation="Use calloc(count, size) (which checks for overflow) or explicitly validate that count * size doesn't overflow before calling malloc()."),
    PatternRule(id='CS-003', title='TLS certificate validation callback always returns true', pattern='ServerCertificateValidationCallback\\s*=.*=>\\s*true', language='csharp',
                severity=Severity('Critical'), confidence=Confidence('High'),
                standards=_std('CWE-295', 'A02:2021-Cryptographic Failures', 'ASVS 9.1.1', 'CAPEC-94', 'PW.5.1'),
                remediation='Implement real certificate chain validation instead of a callback that unconditionally returns true.'),
    PatternRule(id='CS-004', title='Weak cryptographic primitive (MD5 or DES)', pattern='MD5\\.Create\\s*\\(\\)|new\\s+DESCryptoServiceProvider', language='csharp',
                severity=Severity('Medium'), confidence=Confidence('High'),
                standards=_std('CWE-327', 'A02:2021-Cryptographic Failures', 'ASVS 6.2.1', 'CAPEC-97', 'PW.6.1'),
                remediation='Use SHA256.Create() (or better) for hashing and Aes.Create() for symmetric encryption.'),
    PatternRule(id='CS-005', title='Open redirect via unvalidated query string value', pattern='Redirect\\s*\\(\\s*Request\\.QueryString\\[', language='csharp',
                severity=Severity('Medium'), confidence=Confidence('Medium'),
                standards=_std('CWE-601', 'A01:2021-Broken Access Control', 'ASVS 5.1.5', 'CAPEC-178', 'PW.5.1'),
                remediation='Validate the redirect target against an allow-list before passing it to Redirect().'),
    PatternRule(id='CS-006', title='Anti-forgery token validation explicitly skipped', pattern='\\[IgnoreAntiforgeryToken\\]', language='csharp',
                severity=Severity('High'), confidence=Confidence('High'),
                standards=_std('CWE-352', 'A01:2021-Broken Access Control', 'ASVS 4.2.2', 'CAPEC-62', 'PW.5.1'),
                remediation="Confirm the action doesn't perform a state-changing operation reachable from an authenticated browser session before ignoring the anti-forgery token."),
    PatternRule(id='SW-001', title='App Transport Security disabled (arbitrary loads allowed)', pattern='NSAllowsArbitraryLoads.{0,20}true', language='swift',
                severity=Severity('High'), confidence=Confidence('High'),
                standards=_std('CWE-295', 'A02:2021-Cryptographic Failures', 'ASVS 9.1.1', 'CAPEC-94', 'PW.5.1'),
                remediation='Remove the ATS exception, or scope it to a specific, justified domain (NSExceptionDomains) instead of allowing arbitrary insecure loads app-wide.'),
    PatternRule(id='SW-002', title='MD5 used via CommonCrypto', pattern='CC_MD5\\s*\\(', language='swift',
                severity=Severity('Medium'), confidence=Confidence('High'),
                standards=_std('CWE-327', 'A02:2021-Cryptographic Failures', 'ASVS 6.2.1', 'CAPEC-97', 'PW.6.1'),
                remediation="Use CryptoKit's SHA256 (or better) instead of CC_MD5."),
    PatternRule(id='SW-003', title='SQLite query built with string interpolation', pattern='sqlite3_exec\\([^)]*\\\\\\(', language='swift',
                severity=Severity('Critical'), confidence=Confidence('Medium'),
                standards=_std('CWE-89', 'A03:2021-Injection', 'ASVS 5.3.4', 'CAPEC-66', 'PW.5.1'),
                remediation='Use sqlite3_prepare_v2 with bound parameters (sqlite3_bind_*) instead of interpolating values into the SQL string.'),
    PatternRule(id='SW-004', title='WebView loads a non-literal (dynamic) URL directly', pattern='webView\\.load\\s*\\(\\s*URLRequest\\(url:\\s*\\w+\\)\\)', language='swift',
                severity=Severity('Medium'), confidence=Confidence('Low'),
                standards=_std('CWE-601', 'A01:2021-Broken Access Control', 'ASVS 5.1.5', 'CAPEC-178', 'PW.5.1'),
                remediation='Validate the URL against an allow-list of expected hosts/schemes before loading it in a WebView.'),
    PatternRule(id='SC-001', title='SQL statement executed with an interpolated string', pattern='(?:statement|stmt)\\.execute(?:Query|Update)?\\s*\\(\\s*s[\\"\']', language='scala',
                severity=Severity('Critical'), confidence=Confidence('Medium'),
                standards=_std('CWE-89', 'A03:2021-Injection', 'ASVS 5.3.4', 'CAPEC-66', 'PW.5.1'),
                remediation='Use a PreparedStatement with bound parameters, or Slick\'s parameterized sql"..." interpolator, instead of an interpolated s"..." string.'),
    PatternRule(id='SC-002', title='Shell command built with string interpolation and executed via sys.process', pattern='s[\\"\'][^\\"\']*\\$\\{[^}]+\\}[^\\"\']*[\\"\']\\s*\\.!!?', language='scala',
                severity=Severity('Critical'), confidence=Confidence('Medium'),
                standards=_std('CWE-78', 'A03:2021-Injection', 'ASVS 5.2.8', 'CAPEC-88', 'PW.5.1'),
                remediation='Avoid building shell command strings with interpolation; pass arguments as a Seq[String] to scala.sys.process.Process instead of a single interpolated command line.'),
    PatternRule(id='SC-003', title='Java ObjectInputStream used to deserialize data', pattern='new\\s+ObjectInputStream\\s*\\(', language='scala',
                severity=Severity('Critical'), confidence=Confidence('Low'),
                standards=_std('CWE-502', 'A08:2021-Software and Data Integrity Failures', 'ASVS 5.5.3', 'CAPEC-586', 'PW.4.1'),
                remediation='Avoid Java serialization for untrusted data; use a safe format (JSON/protobuf) with an explicit schema instead.'),
    PatternRule(id='SC-004', title='MD5 used for hashing', pattern='MessageDigest\\.getInstance\\s*\\(\\s*[\\"\']MD5[\\"\']\\)', language='scala',
                severity=Severity('Medium'), confidence=Confidence('High'),
                standards=_std('CWE-327', 'A02:2021-Cryptographic Failures', 'ASVS 6.2.1', 'CAPEC-97', 'PW.6.1'),
                remediation='Use SHA-256 (or better) instead of MD5.'),
    PatternRule(id='PH-016', title='Weak PRNG (rand/mt_rand) used for a token or password', pattern='(?:token|password)\\s*=\\s*[^;]*\\b(?:mt_)?rand\\(\\)', language='php',
                severity=Severity('Medium'), confidence=Confidence('Medium'),
                standards=_std('CWE-330', 'A02:2021-Cryptographic Failures', 'ASVS 6.3.1', 'CAPEC-112', 'PW.5.1'),
                remediation='Use random_bytes() or random_int() (CSPRNG) instead of rand()/mt_rand() for anything security-sensitive.'),
    PatternRule(id='PH-017', title='LDAP search filter built from unsanitized request data', pattern='ldap_search\\([^)]*\\$_(?:GET|POST|REQUEST)', language='php',
                severity=Severity('Critical'), confidence=Confidence('Medium'),
                standards=_std('CWE-90', 'A03:2021-Injection', 'ASVS 5.3.8', 'CAPEC-136', 'PW.5.1'),
                remediation='Escape special LDAP filter characters (ldap_escape()) in any user-supplied value before building the search filter.'),
    PatternRule(id='JV-014', title='SecureRandom seeded with a fixed literal value', pattern='new\\s+SecureRandom\\s*\\(\\s*\\d', language='java',
                severity=Severity('Medium'), confidence=Confidence('High'),
                standards=_std('CWE-330', 'A02:2021-Cryptographic Failures', 'ASVS 6.3.1', 'CAPEC-112', 'PW.5.1'),
                remediation='Use the no-argument SecureRandom() constructor — seeding it with a fixed number makes its output predictable, defeating the point of using SecureRandom.'),
    PatternRule(id='GO-012', title='LDAP search filter built with string concatenation', pattern='ldap\\.(?:Search|NewSearchRequest)\\([^)]*\\+', language='go',
                severity=Severity('Medium'), confidence=Confidence('Low'),
                standards=_std('CWE-90', 'A03:2021-Injection', 'ASVS 5.3.8', 'CAPEC-136', 'PW.5.1'),
                remediation='Escape special LDAP filter characters in any user-supplied value before building the search filter.'),
    PatternRule(id='SW-005', title='Keychain item stored with kSecAttrAccessibleAlways', pattern='kSecAttrAccessibleAlways\\b', language='swift',
                severity=Severity('Medium'), confidence=Confidence('High'),
                standards=_std('CWE-922', 'A02:2021-Cryptographic Failures', 'ASVS 6.4.2', 'CAPEC-150', 'PW.5.1'),
                remediation='Use kSecAttrAccessibleWhenUnlockedThisDeviceOnly (or another *ThisDeviceOnly variant) unless the item genuinely needs to be readable before the device is unlocked and included in backups.'),
    PatternRule(id='SW-006', title='Sensitive-looking value written to the general pasteboard', pattern='UIPasteboard\\.general\\.string\\s*=', language='swift',
                severity=Severity('Low'), confidence=Confidence('Low'),
                standards=_std('CWE-200', 'A01:2021-Broken Access Control', 'ASVS 8.3.2', 'CAPEC-118', 'PW.5.1'),
                remediation="Avoid putting secrets/tokens on the general pasteboard — other apps can read it. If copy-to-clipboard is required, consider expiring pasteboard content (UIPasteboard's expirationDate option)."),
    PatternRule(id='SW-007', title='String(format:) called with a non-literal format string', pattern='String\\(format:\\s*\\w+,', language='swift',
                severity=Severity('Medium'), confidence=Confidence('Low'),
                standards=_std('CWE-134', 'A03:2021-Injection', 'ASVS 5.3.7', 'CAPEC-135', 'PW.5.1'),
                remediation='Pass a literal format string and put dynamic values in the arguments, not in the format string itself.'),
    PatternRule(id='SC-005', title='Raw Html() wrapper applied to request-derived data', pattern='\\bHtml\\(\\s*(?:request|input|params)\\b', language='scala',
                severity=Severity('High'), confidence=Confidence('Low'),
                standards=_std('CWE-79', 'A03:2021-Injection', 'ASVS 5.3.3', 'CAPEC-63', 'PW.5.1'),
                remediation="Let Twirl's default HTML escaping handle output; only wrap known-safe, sanitized content in Html(), never raw request data."),
]


# ---- batch 3 (session 3): additional language-specific rules ----
PATTERN_RULES += [
    PatternRule(id='RB-011', title='Insecure YAML.load without safe mode', pattern='YAML\\.load\\s*\\((?!.*safe)', language='ruby',
                severity=Severity('Critical'), confidence=Confidence('Medium'),
                standards=_std('CWE-502', "", "", "", ""),
                remediation='Use YAML.safe_load instead of YAML.load (Psych 4+ made safe_load the default, but older Ruby/Psych versions need it explicitly).'),
    PatternRule(id='JV-015', title='Math.random() used for a token or password', pattern='(?:token|password)\\s*=\\s*[^;]*Math\\.random\\(\\)', language='java',
                severity=Severity('High'), confidence=Confidence('Medium'),
                standards=_std('CWE-330', "", "", "", ""),
                remediation='Use java.security.SecureRandom instead of Math.random() for anything security-sensitive.'),
    PatternRule(id='GO-013', title='JWT signed with a hardcoded secret', pattern='SignedString\\(\\[\\]byte\\(\\s*[\\"\']', language='go',
                severity=Severity('Critical'), confidence=Confidence('Medium'),
                standards=_std('CWE-798', "", "", "", ""),
                remediation='Load the signing secret from environment configuration or a secrets manager, never as a literal in source.'),
    PatternRule(id='KT-006', title='JavaScript interface exposed to a WebView', pattern='addJavascriptInterface\\s*\\(', language='kotlin',
                severity=Severity('High'), confidence=Confidence('Medium'),
                standards=_std('CWE-749', "", "", "", ""),
                remediation='Avoid exposing native interfaces to WebView JavaScript unless the loaded content is fully trusted; if required, restrict it to specific methods with @JavascriptInterface and validate the WebView only ever loads trusted origins.'),
    PatternRule(id='CS-007', title='SqlCommand built with string concatenation', pattern='new\\s+SqlCommand\\(\\s*[\\"\'][^\\"\']*[\\"\']\\s*\\+', language='csharp',
                severity=Severity('Critical'), confidence=Confidence('Medium'),
                standards=_std('CWE-89', "", "", "", ""),
                remediation='Use parameterized queries (SqlParameter / @paramName placeholders) instead of concatenating values into the command text.'),
]
...
PATTERN_RULES += [
    PatternRule(id="PH-013", title="PHP shell_exec injection", language="php",
    ...
    PatternRule(id="PH-014", title="PHP system() injection", language="php",
                pattern=r"\bsystem\s*\(\s*\$_(GET|POST|REQUEST)",
                severity=Severity.CRITICAL, standards=STD_CMDI, confidence=Confidence.HIGH,
                remediation="Avoid system() with user input; if necessary use escapeshellarg()."),
    PatternRule(id="PH-015", title="PHP XSS via echo without escape", language="php",
    ...
    PatternRule(id="JS-017", title="Node.js SQL injection via template literal", language="javascript",
                pattern=r"(query|execute|db\.run)\s*\(\s*`[^`]*\$\{",
                severity=Severity.CRITICAL, standards=STD_SQLI, confidence=Confidence.MEDIUM,
                remediation="Use parameterized queries or a query builder; never interpolate variables."),
    PatternRule(id="JS-018", title="Node.js path traversal via path.join with req", language="javascript",
    ...
    PatternRule(id="JS-019", title="Node.js SSRF via axios with user input", language="javascript",
    ...
    PatternRule(id="JV-012", title="Java SQL injection via Statement.execute", language="java",
    ...
    PatternRule(id="JV-013", title="Java command injection via Runtime.exec with concat", language="java",
                pattern=r"Runtime\.getRuntime\(\)\.exec\s*\([^)]*\+",
                severity=Severity.CRITICAL, standards=STD_CMDI, confidence=Confidence.HIGH,
                remediation="Pass command arguments as a String[] array, never as a concatenated string."),
    PatternRule(id="GO-010", title="Go SQL injection via Sprintf in query", language="go",
    ...
    PatternRule(id="GO-011", title="Go path traversal via filepath.Join with user data", language="go",
    ...
    PatternRule(id="C-006", title="Unsafe C sprintf without bounds", language="c",
    ...
    PatternRule(id="RB-010", title="Ruby mass assignment via params.permit!", language="ruby",
...
# ---- batch 3 (session 3): additional Dockerfile checks ----
DOCKERFILE_CHECKS += [
    {"id": 'DCK-016', "title": 'Cloud credentials directory copied into image', "severity": 'Critical', "category": 'Secrets',
     "pattern": 'COPY\\s+.*\\.(?:aws|kube|gcloud|azure)\\b',
     "detail": 'Cloud provider credential/config directories are being baked into the image layer, permanently embedding them in the image history.',
     "rec": "Never copy credential directories into an image; mount them at runtime (or better, use the platform's native workload-identity mechanism) instead."},
    {"id": 'DCK-017', "title": 'TLS certificate verification disabled via environment variable', "severity": 'High', "category": 'Crypto',
     "pattern": '(?:NODE_TLS_REJECT_UNAUTHORIZED\\s*=\\s*[\\"\']?0|PYTHONHTTPSVERIFY\\s*=\\s*0|GIT_SSL_NO_VERIFY\\s*=\\s*true)',
     "detail": 'An environment variable that globally disables TLS certificate verification is baked into the image.',
     "rec": "Remove the override and fix the underlying certificate/CA trust issue (e.g. add an internal CA to the image's trust store) instead."},
    {"id": 'DCK-018', "title": 'Overly permissive chmod (777)', "severity": 'Medium', "category": 'Permissions',
     "pattern": 'chmod\\s+(?:-R\\s+)?777\\b',
     "detail": 'chmod 777 grants read/write/execute to every user on the system for the affected files.',
     "rec": 'Grant the minimum permissions actually needed (e.g. 750 or 640), and prefer a non-root USER over widening file permissions.'},
    {"id": 'DCK-019', "title": 'apt-get install without --no-install-recommends', "severity": 'Low', "category": 'Hygiene',
     "pattern": 'apt-get\\s+install(?!.*--no-install-recommends)',
     "detail": 'Without --no-install-recommends, apt pulls in extra recommended packages, growing the image and its attack surface unnecessarily.',
     "rec": 'Add --no-install-recommends to apt-get install calls.'},
    {"id": 'DCK-020', "title": 'HEALTHCHECK explicitly disabled', "severity": 'Low', "category": 'Resilience',
     "pattern": 'HEALTHCHECK\\s+NONE',
     "detail": 'HEALTHCHECK NONE explicitly disables even a healthcheck inherited from a base image.',
     "rec": 'Define a real HEALTHCHECK instead of explicitly disabling one, unless the orchestrator (e.g. Kubernetes liveness probes) already covers this.'},
    {"id": 'DCK-021', "title": 'No WORKDIR instruction set', "severity": 'Info', "category": 'Hygiene',
     "pattern": None, "check_fn": lambda c: "WORKDIR" not in c,
     "detail": "Without WORKDIR, RUN/CMD/COPY operate relative to whatever the base image's default directory is, which is easy to get wrong across base-image changes.",
     "rec": 'Set an explicit WORKDIR early in the Dockerfile.'},
    {"id": 'DCK-022', "title": 'Database port exposed directly', "severity": 'Medium', "category": 'Network',
     "pattern": 'EXPOSE\\s+(?:3306|5432|27017|6379|9200|1433)\\b',
     "detail": 'A well-known database port is exposed directly from the container, suggesting the database may be reachable without going through an application tier.',
...
# ---- batch 3 (session 3): additional Compose checks ----
COMPOSE_CHECKS += [
    {"id": 'CMP-009', "title": 'Image uses :latest tag (no version pin)', "severity": 'Medium', "category": 'Hygiene',
     "pattern": 'image:\\s*[^\\s:]+(?::latest)?\\s*$',
     "detail": 'Using :latest (or no tag, which defaults to it) means the exact image contents can change under you between deployments.',
     "rec": 'Pin a specific version tag (or better, a content digest) for every image.'},
    {"id": 'CMP-010', "title": 'Host root filesystem mounted as a volume', "severity": 'Critical', "category": 'Privilege',
     "pattern": '-\\s*[\\"\']?/:/',
     "detail": "Mounting the host's root filesystem into the container gives the container full read/write access to the host filesystem.",
     "rec": 'Mount only the specific host directories the workload actually needs, never the root filesystem.'},
    {"id": 'CMP-011', "title": 'Dangerous Linux capability added', "severity": 'Critical', "category": 'Privilege',
     "pattern": 'cap_add:\\s*[\\s\\S]{0,60}(?:SYS_ADMIN|NET_ADMIN|ALL)',
     "detail": 'The service requests a highly privileged Linux capability via cap_add.',
     "rec": 'Drop all capabilities by default (cap_drop: [ALL]) and add back only the specific, narrow capability actually needed.'},
    {"id": 'CMP-012', "title": 'Secret-like value passed as a command-line argument', "severity": 'Medium', "category": 'Secrets',
     "pattern": 'command:\\s*[\\s\\S]{0,100}--(?:password|token|secret|api-?key)[=\\s]',
     "detail": 'Secrets passed as command-line arguments are visible to anyone who can run `docker inspect` or read /proc/<pid>/cmdline on the host.',
     "rec": 'Pass secrets via environment variables sourced from a secrets store, or Docker secrets, instead of command-line flags.'},
    {"id": 'CMP-013', "title": 'Interactive shell access left enabled (stdin_open + tty)', "severity": 'Low', "category": 'Hygiene',
...
K8S_CHECKS += [
    ...
    {"id": "K8S-011", "title": "Dangerous Linux capability added", "severity": "Critical", "category": "Privilege",
     "pattern": r"add:\s*\[[^\]]*(?:SYS_ADMIN|NET_ADMIN|ALL)",
     "detail": "The container requests a highly privileged Linux capability (SYS_ADMIN/NET_ADMIN/ALL), any of which can lead to full host compromise.",
     "rec": "Drop ALL capabilities by default and add back only the specific, narrow capability the container actually needs."},

    {"id": "K8S-012", "title": "hostPID: true", "severity": "High", "category": "Privilege",
     "pattern": r"hostPID:\s*true",
     "detail": "The pod shares the host's PID namespace, letting it see (and potentially signal/kill) every process on the node.",
     "rec": "Remove hostPID: true unless the workload is a genuine node-level debugging/monitoring tool."},

    {"id": "K8S-013", "title": "hostIPC: true", "severity": "High", "category": "Privilege",
     "pattern": r"hostIPC:\s*true",
     "detail": "The pod shares the host's IPC namespace, exposing host shared-memory segments and semaphores to the container.",
     "rec": "Remove hostIPC: true unless there's a specific, justified need for host IPC access."},

    {"id": "K8S-014", "title": "RBAC rule grants wildcard verbs or resources", "severity": "Critical", "category": "Privilege",
     "pattern": r"(?:verbs|resources):\s*\[\s*[\"']\*[\"']",
     "detail": "A Role/ClusterRole grants '*' for verbs or resources, which is effectively unrestricted access within its scope.",
     "rec": "List the specific verbs (get, list, watch, ...) and resource types the workload actually needs."},

    {"id": "K8S-015", "title": "RBAC binding grants cluster-admin", "severity": "Critical", "category": "Privilege",
     "pattern": None, "check_fn": lambda c: "ClusterRoleBinding" in c and "cluster-admin" in c,
     "detail": "A ClusterRoleBinding grants the cluster-admin role, giving full control over every resource in every namespace.",
     "rec": "Bind to a narrowly-scoped Role/ClusterRole instead; reserve cluster-admin for actual cluster operators, not workloads."},

    {"id": "K8S-016", "title": "No seccomp profile set", "severity": "Low", "category": "Privilege",
     "pattern": None, "check_fn": lambda c: "seccompProfile" not in c,
     "detail": "Without a seccomp profile, the container has access to the full Linux syscall surface.",
     "rec": "Set securityContext.seccompProfile.type: RuntimeDefault (or a tighter custom profile)."},

    {"id": "K8S-017", "title": "Secret-shaped key stored in a ConfigMap instead of a Secret", "severity": "High", "category": "Secrets",
     "pattern": None,
     "check_fn": lambda c: "kind: ConfigMap" in c and bool(re.search(r"(?i)(password|secret|token|api[_-]?key)\s*:", c)),
     "detail": "A ConfigMap (unencrypted, not access-restricted the way Secrets are) contains a key named like a credential.",
     "rec": "Move credential-shaped values into a Secret object; ConfigMaps are for non-sensitive configuration only."},

    {"id": "K8S-018", "title": "No liveness or readiness probe defined", "severity": "Medium", "category": "Resilience",
     "pattern": None, "check_fn": lambda c: "livenessProbe" not in c and "readinessProbe" not in c,
     "detail": "Without health probes, Kubernetes can't detect a hung/unready container and route traffic or restart accordingly.",
